...
struct avl_tree
{
    ...
    avl_tree(int(*Compare)(T a, T b)) : root(nullptr) , elements(0)
    {
        if(Compare == nullptr)
        {
            throw ("Bad");
    ...
    ~avl_tree()
    {
        ...
        Deque<avl_node<T>*> stackMAX;

        while(max != nullptr || !stackMAX.isEmpty())
        {   
            while(max != nullptr)
            {
                stackMAX.addTail(max);
                max = max->left;
            }

            max = stackMAX.removeTail();
            temp = max->right;
            delete max;
