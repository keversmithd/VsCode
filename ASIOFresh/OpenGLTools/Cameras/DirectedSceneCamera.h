...
cf_bvol dsc_DefaultCamera = {glm::vec3(0.0,0.0,0.0), glm::vec3(0.0,0.0,0.0)};
...
struct DirectedSceneCamera
{
    ...
    DirectedSceneCamera() : trackedObject(&dsc_DefaultCamera)
    {
        super.self = this;
    ...
    void SetRelativeLocation(const glm::vec3 _displacement)
    {   
    ...
    void SetAngularLocation(const glm::vec3 radians, const float radius)
    {
        glm::vec3 trackedCenter;
        bvol_center(*trackedObject, trackedCenter);
        //Euler not spherical
        ...
        // cameraPos.x += cameraPos.x + radius*upVector.x*cosf(radians.y) + radius*upVector.z*sin(radians.y) + radius*upVector.x*cosf(radians.z) - radius*upVector.y*sinf(radians.z);
        // cameraPos.y += radius * cosf(radians.x) - upVector.z * radius * cosf(radians.x) + upVector.y + radius*upVector.x*sinf(radians.z) + radius*upVector.y*cosf(radians.z);
        // cameraPos.z += radius * upVector.y * sinf(radians.x) + upVector.z * radius * cosf(radians.x) - radius*upVector.x*sinf(radians.y) + radius*upVector.z*cosf(radians.y) + upVector.z;

        displacement.x = radius * (upVector.x * cosf(radians.y) + upVector.z * sinf(radians.y) * sinf(radians.z) - upVector.y * cosf(radians.z) * sinf(radians.y));
        displacement.y = radius * (upVector.y * cosf(radians.x) + upVector.z * sinf(radians.x) * sinf(radians.z) + upVector.x * cosf(radians.z) * sinf(radians.x));
        displacement.z = radius * (upVector.z * cosf(radians.x) * cosf(radians.y) - upVector.y * cosf(radians.x) * sinf(radians.z) + upVector.x * cosf(radians.x) * cosf(radians.z));
    }
...
void dsc_proccessInput(void* self, GLFWwindow* window)
{
    ...
    static glm::vec3 dsc_centerVolume;
    bvol_center(*s->trackedObject, dsc_centerVolume);

    static glm::vec3 dsc_direction;

    s->cameraPos = dsc_centerVolume + s->displacement;

    dsc_direction = dsc_centerVolume-s->cameraPos;
    ...
    s->matricies.view = glm::lookAt(s->cameraPos, s->cameraPos + dsc_direction, s->cameraUp);
