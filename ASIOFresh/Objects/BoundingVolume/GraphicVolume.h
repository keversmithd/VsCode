...
struct GraphicVolume
{
    ...
    void WriteBoxDown()
    {
        ...
        FILE *mesh_eb = getFileWithDirectory("Meshes/cubeler.mb", 1, "wb", &directoryFetchError);
        ...
        fwrite(memoryData, sizeof(float), memoryDataBack, mesh_eb);
        fwrite(&elementDataBack, sizeof(int), 1, mesh_eb);
        fwrite(elementData, sizeof(unsigned int), elementDataBack, mesh_eb);
