...
int width, pondwidth, height, pondheight, nrChannels;
unsigned char *data = stbi_load(WS "frog.jpg", &width, &height, &nrChannels, 0); 
unsigned char *ponddata = stbi_load(WS "pond.jpg", &pondwidth, &pondheight, &nrChannels, 0);
if(!data || !ponddata){return -1;}

NGLTexture texture, pondtexture;
texture.GenerateTexture();
pondtexture.GenerateTexture();
...
pondtexture.BindTexture(GL_TEXTURE_2D);
glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);	
glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
pondtexture.TexImage2D(GL_TEXTURE_2D, 0, GL_RGB, pondwidth, pondheight, 0, GL_RGB, GL_UNSIGNED_BYTE, ponddata);
glGenerateMipmap(GL_TEXTURE_2D);
stbi_image_free(data);
stbi_image_free(ponddata);
#pragma endregion

#pragma region Shaders

    int error = 0;
    NGLProgram nglProgram = CreateShader(WS "shaders/goroudDMap.vs", WS "shaders/goroudDMap.vs");
...
#pragma endregion
        ...
        setVec3(nglProgram,"light.ambient", lightambient);
        setVec3(nglProgram,"light.diffuse", lightdiffuse);
        setVec3(nglProgram,"light.specular", lightspecular);
        setFloat(nglProgram,"light.shininess", shininess);

        setInt(nglProgram, "material.diffuse", 0);
        setInt(nglProgram, "material.specular", 1);
        ...
        ImGui_ImplGlfwGL3_NewFrame();
            {
                ...
                ImGui::InputFloat("shiny", &shininess);

                ImGui::SliderFloat3("lightambient", glm::value_ptr(lightambient),0, 2);
                ImGui::SliderFloat3("lightdiffuse", glm::value_ptr(lightdiffuse),0, 2);
                ImGui::SliderFloat3("lightspecular", glm::value_ptr(lightspecular),0, 2);
