...
void PinJointedStiffness(GEmat& StiffnessMatrix, double EA, GEmat& Coordinates)
{

    int Dimension = Coordinates.MATRIX_DIMENSION_X;
    ...
    float zl = 0;
    if(Dimension == 1){
        ElementLength=Coordinates(1,0)-Coordinates(0,0);
        StiffnessMatrix(0,0)=1.0f;
        StiffnessMatrix(0,1)=-1.0f;
        StiffnessMatrix(1,0)=-1.0f;
        StiffnessMatrix(1,1)=1.0f;
    ...
    }else if(Dimension == 2)
    {
        ...
        x2=Coordinates(1,0);
        y2=Coordinates(1,1);
        ElementLength = sqrt(((y2-y1)*(y2-y1)) + ((x2-x1)*(x2-x1)));
        xspace = (x2-x1)/ElementLength;
        yspace = (y2-y1)/ElementLength;
        a=xspace*xspace;
        b=yspace*yspace;
        c=xspace*yspace;
        StiffnessMatrix(0,0)=a;
        StiffnessMatrix(2,2)=a;
        StiffnessMatrix(0,2)=-a;
        StiffnessMatrix(2,0)=-a;
        StiffnessMatrix(1,1)=b;
        StiffnessMatrix(3,3)=b;
        StiffnessMatrix(1,3)=-b;
        StiffnessMatrix(3,1)=-b;
        StiffnessMatrix(0,1)=c;
        StiffnessMatrix(1,0)=c;
        StiffnessMatrix(2,3)=c;
        StiffnessMatrix(3,2)=c;
        StiffnessMatrix(0,3)=-c;
        StiffnessMatrix(3,0)=-c;
        StiffnessMatrix(1,2)=-c;
        StiffnessMatrix(2,1)=-c;
    }else if(Dimension == 3)
    {
        x1=Coordinates(0,0);
        y1=Coordinates(0,1);
        ...
        yl=y2-y1;
        zl=z2-z1;
        ElementLength=sqrt(xl*xl+yl*yl+zl*zl);
        xl = xl/ElementLength;
        yl = yl/ElementLength;
        zl = zl/ElementLength;
        a=xl*xl;
        ...
        f=zl*xl;
        StiffnessMatrix(0,0)=a;
        StiffnessMatrix(3,3)=a;
        StiffnessMatrix(1,1)=b;
        StiffnessMatrix(4,4)=b;
        StiffnessMatrix(2,2)=c;
        StiffnessMatrix(5,5)=c;
        StiffnessMatrix(0,1)=d;
        StiffnessMatrix(1,0)=d;
        StiffnessMatrix(3,4)=d;
        StiffnessMatrix(4,3)=d;
        StiffnessMatrix(1,2)=e;
        StiffnessMatrix(2,1)=e;
        StiffnessMatrix(4,5)=e;
        StiffnessMatrix(5,4)=e;
        StiffnessMatrix(0,2)=f;
        StiffnessMatrix(2,0)=f;
        StiffnessMatrix(3,5)=f;
        StiffnessMatrix(5,3)=f;
        StiffnessMatrix(0,3)=-a;
        StiffnessMatrix(3,0)=-a;
        StiffnessMatrix(1,4)=-b;
        StiffnessMatrix(4,1)=-b;
        StiffnessMatrix(2,6)=-c;
        StiffnessMatrix(5,2)=-c;
        StiffnessMatrix(0,4)=-d;
        StiffnessMatrix(4,0)=-d;
        StiffnessMatrix(1,3)=-d;
        StiffnessMatrix(3,1)=-d;
        StiffnessMatrix(1,5)=-e;
        StiffnessMatrix(5,1)=-e;
        StiffnessMatrix(2,4)=-e;
        StiffnessMatrix(4,2)=-e;
        StiffnessMatrix(0,5)=-f;
        StiffnessMatrix(5,0)=-f;
        StiffnessMatrix(2,3)=-f;
        StiffnessMatrix(3,2)=-f;
    }
    StiffnessMatrix.mulBy(EA/ElementLength);
}
void GlobToAxial(float Axial, GEmat& Action, GEmat& Coordinates)
{
    int Dimension = Coordinates.MATRIX_DIMENSION_X;
    float ElementLength = 0.0;
    float Sum = 0;
    ...
    {
        Sum+=pow((Coordinates(1,i)-Coordinates(0,i)),2);
    }
    ElementLength = sqrt(Sum);
    Axial = 0;
    for(int i = 0; i < Dimension; i++)
    {
        Axial+=(Coordinates(1,i)-Coordinates(0,i))/ElementLength*Action(Dimension+i);
    }
}
...
int main()
{
    ...
    int NumberOfDegreesOfFreedom = 2;
    ...
    GEmat Coordinates(NumberOfNodesPerElement,NumberOfDimensions);
    ...
    for(int i = 0; i < NumberOfElements; i++)
    {
        PinJointedStiffness(StiffnessMatrix, ElementProperties(ElementTypes(i)-1), Coordinates(i));
